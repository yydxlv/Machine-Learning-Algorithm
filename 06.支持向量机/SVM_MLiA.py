...
class optStruct:
    def __init__(self,dataMatIn, classLabels, C, toler, kTup):  # Initialize the structure with the parameters
        ...
        self.b = 0
        self.eCache = mat(zeros((self.m,2)))  # first column is valid flag second column is real E value
...
def calcEk(oS, k):  # 计算E值并返回
    ...
    return Ek
# 选择第二个alpha 值 （内循环的alpha值） 确保在每次优化中采用最大步长
...
def testRbf(k1=1.3):
    ...
    print("labelMat:",type(labelMat))    # mat
    svInd=nonzero(alphas.A>0)[0]
    print("svInd:\n", svInd)
    sVs=datMat[svInd]  # get matrix of only support vectors
    print("sVs:\n", sVs)
    labelSV = labelMat[svInd]
    print(type(labelSV),shape(labelSV))               # mat (26,1)
    print(type(alphas[svInd]), shape(alphas[svInd]))  # mat (26,1)
    print(labelSV,alphas[svInd],multiply(labelSV,alphas[svInd]))
